...
class GettyProviderConfigTests:
    ...
    def test_allowed_instance_scopes(self):
        ...
        assert provider.allowed_instance_scopes == ['single', 'threaded_thread']

    def test_override_instance_scopes(self):
        # Default GettyProvider is an AAT provider
        provider = GettyProvider({'id': 'AAT'}, allowed_instance_scopes=['single'])
...
class GettyProviderBasicTests:
    def _get_provider(self):
        return GettyProvider(
            {'id': 'AAT'}, base_url='http://vocab.getty.edu', vocab_id='aat'
        )
    ...
    def test_get_by_uri(self):
        # Default GettyProvider is an AAT provider
        concept = GettyProvider({'id': 'AAT'}).get_by_uri(
...
class AATProviderTests(unittest.TestCase):
    ...
    def test_find_concepts_kerk_zh(self):
        r = AATProvider({'id': 'AAT', 'default_language': 'zh'}).find(
            {'label': 'kerk', 'type': 'concept'}
        )
        self.assertIsInstance(r, list)
        self.assertGreater(len(r), 0)
        for res in r:
    ...
    def test_find_concepts_kerk_language(self):
        kwargs = {'language': 'nl'}
        result = AATProvider({'id': 'AAT'}).find(
            {'label': 'kerk', 'type': 'concept'}, **kwargs
        )
        assert len(result) > 0
        labels = []
        for res in result:
    ...
    def test_find_concepts_kerk(self):
        ...
        r2 = AATProvider({'id': 'AAT', 'default_language': 'en'}).find(
            {'label': 'kirche', 'type': 'concept'}
        )
        r3 = AATProvider({'id': 'AAT', 'default_language': 'nl'}).find(
            {'label': 'kerk', 'type': 'concept'}
        )
    ...
    def test_find_matches(self):
        r = AATProvider({'id': 'AAT'}).find(
            {'matches': {'uri': 'http://id.loc.gov/authorities/subjects/sh85123119'}}
        )
        assert len(r) == 1
    ...
    def test_find_closematches(self):
        r = AATProvider({'id': 'AAT'}).find(
            {
                'matches': {
                    'uri': 'http://id.loc.gov/authorities/subjects/sh85123119',
                    'type': 'close',
                }
            }
        )
    ...
    def test_answer_wrong_query(self):
        with pytest.raises(ProviderUnavailableException):
            provider = GettyProvider(
                {'id': 'test'}, vocab_id='aat', url='http://vocab.getty.edu/aat'
            )
...
class TGNProviderTests:
    ...
    def test_get_top_display(self):
        kwargs = {'language': 'nl'}
        top_TGN_display = TGNProvider(
            {'id': 'TGN', 'default_language': 'en'}
        ...
        top_AAT_display = AATProvider(
            {'id': 'AAT', 'default_language': 'nl'}
        ).get_top_display()
        assert isinstance(top_AAT_display, list)
        assert len(top_AAT_display) > 0
        assert 'Facet Stijlen en perioden' in [
            label['label'] for label in top_AAT_display
        ]
    ...
    def test_get_childeren_display(self):
        kwargs = {'language': 'nl'}
        childeren_tgn_belgie = TGNProvider(
            {'id': 'TGN', 'default_language': 'nl'}
        ...
        for key in ['id', 'type', 'label', 'uri']:
            assert key in keys_first_display
        assert 'Brussels Hoofdstedelijk Gewest' in [
            label['label'] for label in childeren_tgn_belgie
        ]
